...
import { Plus, Search, Factory, Download, Eye, Calendar, Trash2, ClipboardList } from 'lucide-react';
...
export default function ProductionList() {
  ...
  // When an item is selected, auto-fill from BOM template if one exists
  async function handleItemChange(itemId: number, fieldOnChange: (v: number) => void) {
    ...
    try {
      ...
      if (bom && bom.lines.length > 0) {
        const mapped = bom.lines.map(l => ({
          materialType: l.materialType,
          materialId: l.materialId,
          usedQuantity: l.quantity,
        }));
        replace(mapped);
        toast.info(`${mapped.length} material${mapped.length > 1 ? 's' : ''} auto-filled from BOM template`, {
          description: 'You can still adjust quantities before saving.',
          duration: 4000,
        });
      }
    } finally {
  ...
  // Resolve material name for view sheet
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex gap-2">
            <Button
              ...
              onClick={() => downloadCSV('production.csv', filtered.map(p => ({
                Batch: `B-${String(p.id).padStart(4, '0')}`,
                Date: p.productionDate,
                Item: p.itemName,
                Qty: p.producedQuantity,
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <FormField control={form.control} name="itemId" render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      Finished Item <span className="text-destructive">*</span>
                    </FormLabel>
                    <Select
                      onValueChange={v => handleItemChange(Number(v), field.onChange)}
                ...
                <FormField control={form.control} name="producedQuantity" render={({ field }) => (
                  <FormItem>
                    <FormLabel>Quantity Produced <span className="text-destructive">*</span></FormLabel>
              ...
              <div>
                <div className="flex justify-between items-center mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    {loadingBom && (
                      <span className="text-xs text-muted-foreground animate-pulse">Loading BOM…</span>
                    )}
                  </div>
                  <div className="flex gap-2">
                    {/* BOM hint tooltip */}
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              if (bom && bom.lines.length > 0) {
                                ...
                                toast.info('No BOM template found for this item');
                              }
                            } finally {
                              setLoadingBom(false);
                            }
                        ...
                        <div className="col-span-2">
                          <FormField control={form.control} name={`materialUsed.${i}.usedQuantity`} render={({ field: f }) => (
                            <FormItem>
                              ...
                              <FormControl>
                                <Input type="number" step="0.01" className="h-8 text-xs" {...f} />
                              </FormControl>
                            </FormItem>
                          )} />
                        </div>
              ...
              <FormField control={form.control} name="notes" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Optional batch notes..." rows={2} {...field} />
                  </FormControl>
                </FormItem>
              )} />
            ...
            <div className="mt-6 space-y-5">
                    ...
                    <div key={i} className="flex justify-between items-center p-3 bg-muted/20 rounded-lg text-sm">
                      <div>
                        <Badge variant="secondary" className="text-xs mr-2">
                          {m.materialType === 'raw_material' ? 'Raw' : 'Pkg'}
                        </Badge>
              ...
              {viewItem.notes && (
                <div>
                  ...
                  <p className="mt-1">{viewItem.notes}</p>
                </div>
              )}
            </div>
          )}
