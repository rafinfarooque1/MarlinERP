...
export interface ShareKpi {
  ...
  value: string;
  tone?: CardTone;
  /** Optional text used for non-location explanatory hints. */
  hint?: string;
  /** Location rows are rendered as one readable row per location. */
  locationLines?: { label: string; value: string }[];
}
...
export interface DashboardShareReportProps {
  ...
  generatedAt: string;
  cards: ShareKpi[];
  reportMode: 'details' | 'consolidated';
}
...
export const DashboardShareReport = forwardRef<HTMLDivElement, DashboardShareReportProps>(
  function DashboardShareReport({ locationLabel, periodLabel, presetLabel, generatedAt, cards, reportMode }, ref) {
    return (
      <div
        ref={ref}
      ...
      >
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="text-right shrink-0">
            ...
            <p className="mt-1.5 text-[10px] font-semibold uppercase tracking-wide text-slate-400">Date / Period</p>
            <p className="text-xs text-slate-600">{periodLabel}</p>
            <p className="mt-1 text-[10px] font-semibold uppercase tracking-wide text-slate-400">Report Mode</p>
            <p className="text-xs font-semibold text-slate-600">
              {reportMode === 'details' ? 'Details' : 'Consolidated'}
            </p>
          </div>
        </div>

        {/* KPI grid — structured rows keep every location readable. */}
