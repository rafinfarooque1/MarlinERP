...
**Stored lines carry no name/unit — BY DESIGN.** priceBill strips materialName/unit before persisting (names resolve at read time so renames show through). Consequence: any WRITER that needs an item name (stock_ledger reversals, audit rows) must resolve it from the product masters inside its transaction — reading it off the stored line JSON silently produces blank audit rows that name-tagged cleanups then miss. Guarded by the purchase-reversal-ledger-names suite.
