...
- **Dashboard KPI cards are TWO renderers:** phones get a dedicated `MobileSummaryCards` block in Dashboard.tsx (banking-app style: icon+title, auto-fit bold amount, structured breakdown lines, `auto-rows-fr` equal heights); the shared `SummaryCards` renders desktop only (`hidden md:grid md:grid-cols-6`). Change dashboard card content in BOTH places or they drift.
- **FitAmount pattern:** amounts auto-shrink 24→10px by direct style mutation (no re-renders) with a ResizeObserver refit; if 10px still overflows it flips (sticky per value) to Indian compact notation (₹x.xxCr/L) with the exact figure in title/aria-label. Never use `truncate` on money.
- **Semantic pairing over positional:** when a permission can hide a card (Inventory for non-valuation users), the preceding card carries `spanTwo` so later pairs (Cash|Bank, Receivables|Payables, GP|NP) never shift — never rely on trailing-odd-card logic.
