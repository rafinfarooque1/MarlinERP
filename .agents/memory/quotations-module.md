...
- Salesperson & payment-terms masters (Aug 2026): quotations store BOTH the reference (`salesperson_employee_id`) AND a TEXT snapshot (`salesperson` = employee name at save time; `payment_terms` = label text). PDFs/lists/CSV read only the text, so legacy free text and renamed/deleted terms grandfather with zero renderer changes. New employee refs must be active + at the quote's location or HO; an UNCHANGED ref on edit is grandfathered (no re-validation — resigned/moved staff must not lock old quotes). Free text in the body is IGNORED whenever an employee id is sent.
- `quotation_payment_terms` is Settings-managed configuration and deliberately NOT in the factory-reset table list: its seed is migration_log-guarded and would never re-run, leaving the list empty forever. `GET /quotations/salespeople` (registered before `/:id`) is the minimal directory (id/name/branch only) readable with the quotation page right; terms GET is readable by quotation OR settings users, writes settings-only.
