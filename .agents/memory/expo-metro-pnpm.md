...
**How to apply:** if the employee app goes blank and the bundle URL returns JSON instead of JS, check `watchFolders` first, then restart the expo workflow (config changes need a restart; app code hot-reloads). In Replit development, native API traffic must use the Expo-specific development hostname exposed to the QR/development flow, not the ordinary web development hostname; production builds continue to use the deployed API domain.
